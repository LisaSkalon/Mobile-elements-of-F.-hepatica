...
s2c <- read.table(file.path("path_to_file_location/info.txt"), header = TRUE, stringsAsFactors=FALSE)
...
# write results
write.csv(sleuth_significant, file = "path_to_basedir/sleuth_significant.csv")
# table of results
kallisto_table(so, use_filtered=T, normalized=T, include_covariates=T)

# matrix of results
matrix = sleuth_to_matrix(so, "obs_norm", "tpm")
write.csv(matrix, file = "path_to_basedir/res_matrix.csv")
...
write.csv(cor_matrix, file = "path_to_basedir/cor_matrix.csv")

# boxplot of expression of particular TE
plot_bootstrap(so, "name_of_Repeat", units = "est_counts", color_by = "condition", divide_groups=F ) + theme_bw() 


# bootstrap values used in boxplots 
penelope_bootstrap <- get_bootstrap_summary(so, "name_of_repeat", units = "est_counts")
write.csv(penelope_bootstrap, file = "path_to_basedir/name_of_Repeat_bootstrap.csv")
...
# Pairwise comparisons with Wald test
so <- sleuth_wt(so,'conditionjuvenile','full')
so <- sleuth_wt(so,'conditionegg','full')
so <- sleuth_wt(so,'conditionmetacercaria','full')
so <- sleuth_wt(so,'conditionnewly_emerged_juvenile_1h','full')
so <- sleuth_wt(so,'conditionnewly_emerged_juvenile_3h','full')
so <- sleuth_wt(so,'conditionnewly_emerged_juvenile_24h','full')
